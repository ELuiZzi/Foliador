...
public class DefaultPdfCreator implements PdfCreator {
    ...
    public void imprimirFolios(PdfDocument pdfAFoliar, Settings settings) {
        ...
        try (Document document = new Document(pdfAFoliar)) {
            ...
            int foliosPorPagina = Settings.getFoliosPorHoja();
            ...
            for (int pagina = 1; pagina <= totalPaginas; pagina++) {
                ...
                for (int tab = 0; tab < foliosPorPagina; tab++) {
                    int folio = (pagina - 1) * foliosPorPagina * 1 + tab * 1 + settings.getFolioInicial();
